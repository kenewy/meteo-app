...
try :
  ...
  selected_date = st.date_input("Sélectionnez une date", presentday)
  start_date=selected_date.strftime("%Y-%m-%d")
  end_date = selected_date + timedelta(4)
  end_date=end_date.strftime("%Y-%m-%d")
  j=[]
  nj=[]
  j.append(selected_date)
  nj.append(j[0].strftime("%A"))
  for i in range(4):
    j.append(selected_date + timedelta(i+1))
